import { useState } from "react";
import { useNavigate } from "react-router-dom";

// Import phone input from rect-phone-input-2 library
import PhoneInput from "react-phone-input-2";
import "react-phone-input-2/lib/style.css";

export const SetNumber = () => {
  // Navigation
  const navigate = useNavigate();

  // Credentials and error state
  const [ph, setPh] = useState("");
  const [errors, setErrors] = useState<{ ph?: string }>({});

  // Validation Config Obj
  const validationConfig = {
    ph: { required: true, message: "Phone number is required" },
  };

  // Validation Function
  const validate = () => {
    const errorsData: { ph?: string } = {};

    if (validationConfig.ph.required && !ph) {
      errorsData.ph = validationConfig.ph.message;
    }

    setErrors(errorsData);
    return errorsData;
  };

  // Handle Submit Eventlistener
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const errorsData = validate();
    if (!Object.values(errorsData).length) {
      //Checks if every field is filled if it is then redirect to number verification page
      navigate("/signup/set-job-description");
    }
  };

  return (
    <section className="set-number">
      <section className="px-5 flex flex-col bg-[#F5F5F5] items-center justify-center py-10 min-h-screen gap-5 signup-set-name">
        <h3 className="text-[#1976D2] mb-5 text-2xl sm:text-3xl font-semibold">
          Sign Up
        </h3>
        <div className="w-full max-w-md p-8 bg-white rounded-lg">
          <form className="mx-auto" onSubmit={handleSubmit}>
            <label
              htmlFor="phone"
              className="md:text-lg font-medium text-[#3A3A3A]"
            >
              Mobile Number
            </label>
            {/* Custom Phone Input From rect-phone-input-2 library */}
            <PhoneInput
              inputStyle={{
                paddingBlock: "25px",
              }}
              containerClass="my-4"
              inputClass="!w-full bg-red-500 !border-none shadow-[0px_2px_15px_0px_rgba(0,0,0,0.1)]"
              buttonClass="!border-none shadow-[0px_2px_15px_0px_rgba(0,0,0,0.1)] bg-white"
              searchStyle={{ paddingBlock: "5px", width: "85%" }}
              enableSearch={true}
              inputProps={{
                name: "phone",
                id: "phone",
              }}
              country={"bd"}
              value={ph}
              onChange={setPh}
            />
            <p className="absolute -mt-2 text-xs text-red-500 md:text-sm">
              {errors.ph}
            </p>
            <div className="flex justify-end gap-4 mt-10 controls-button">
              <button
                onClick={() => navigate("/signup/set-name")}
                className="prev-btn ease-linear duration-200 bg-[#CDCDCD] hover:bg-[#A9A9A9] py-3 sm:py-4 px-4 sm:px-6 rounded"
              >
                <svg
                  className="w-6 sm:w-8"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 34 16"
                  fill="none"
                >
                  <path
                    d="M1.277 8.707a1 1 0 010-1.414L7.641.929a1 1 0 011.414 1.414L3.4 8l5.656 5.657a1 1 0 11-1.414 1.414L1.277 8.707zM34 9H1.984V7H34v2z"
                    fill="#535353"
                  />
                </svg>
              </button>
              <button
                type="submit"
                className="next-btn bg-[#1976D2] hover:bg-[#1660A5] ease-linear duration-200 py-3 sm:py-4 px-4 sm:px-6 rounded"
              >
                <svg
                  className="w-6 sm:w-8"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 34 16"
                  fill="none"
                >
                  <path
                    d="M32.723 8.707a1 1 0 000-1.414L26.359.929a1 1 0 10-1.414 1.414L30.6 8l-5.656 5.657a1 1 0 101.414 1.414l6.364-6.364zM0 9h32.016V7H0v2z"
                    fill="#fff"
                  />
                </svg>
              </button>
            </div>
          </form>
        </div>
      </section>
    </section>
  );
};
